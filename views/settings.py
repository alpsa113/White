...
def render() -> None:
    ...
    # ==================================================================== #
    # [중요] 이 페이지의 모든 위젯은 "위젯 전용 key(_xxx_widget)"와 "실제 상태
    # key(simulate, person_ratio, show_clock 등)"를 분리하고, on_change 콜백으로
    # 값을 복사하는 패턴을 씁니다.
    #
    # 이유: 이 위젯들은 설정 페이지에서만 그려지는데, Streamlit은 특정 실행에서
    # 그려지지 않는 위젯의 key를 session_state에서 삭제합니다. 만약 위젯 key를
    # 다른 파일(예: ui/layout.py의 시계, services/detection.py의 데모 로직)에서
    # 직접 참조하면, 페이지를 옮겨 다닐 때 그 값이 사라지거나 초기화되는 문제가
    # 생깁니다. 위젯 key와 실제 상태 key를 분리해두면, 위젯이 화면에서 사라져도
    # 실제 상태값은 항상 안정적으로 유지됩니다.
    # ==================================================================== #
