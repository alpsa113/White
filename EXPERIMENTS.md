...
- 동일한 평가 지표 사용: class별 AP50, mAP50, validation loss
- 동일한 검증 데이터로 최종 checkpoint 평가

성능 비교에서는 `best.pt` 기준 결과와 `final.pt` 기준 결과를 모두 기록하되, 모델 선택 판단은 주로 같은 검증셋의 `best.pt` mAP50을 기준으로 합니다.
...
python train.py --phase 1 --model-cfg configs/model.yaml --phase-cfg configs/phases_rgb_only.yaml --save-dir checkpoints_rgb_only
python train.py --phase 3 --model-cfg configs/model.yaml --phase-cfg configs/phases_rgb_only.yaml --save-dir checkpoints_rgb_only --init-from checkpoints_rgb_only/phase1/best.pt
```
...
```bash
python train.py --phase 1 --model-cfg configs/model_yolo11m.yaml --phase-cfg configs/phases_yolo11m.yaml --save-dir checkpoints_yolo11m
python train.py --phase 2 --model-cfg configs/model_yolo11m.yaml --phase-cfg configs/phases_yolo11m.yaml --save-dir checkpoints_yolo11m --init-from checkpoints_yolo11m/phase1/best.pt
python train.py --phase 3 --model-cfg configs/model_yolo11m.yaml --phase-cfg configs/phases_yolo11m.yaml --save-dir checkpoints_yolo11m --init-from checkpoints_yolo11m/phase2/best.pt
```

## 결과 기록 양식

| 실험 | 백본 | 입력 | Phase | best mAP50 | AP_person | AP_boar | AP_deer | AP_non_target | 비고 |
|---|---|---|---:|---:|---:|---:|---:|---:|---|
...
- RGB-only 대비 DualYOLO의 mAP50과 class별 AP가 높으면 멀티모달 구조의 이점을 주장할 수 있습니다.
- YOLO11m 대비 YOLO26m의 mAP50과 class별 AP가 높으면 YOLO26m 백본 선택의 근거가 됩니다.
