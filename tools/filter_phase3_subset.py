#!/usr/bin/env python3
"""gop_raw에서 phase3 fine-tuning용 축소 학습셋을 생성.

원본 데이터는 수정하지 않고, 선택된 이미지와 라벨만 새 디렉토리로 복사한다.
phase3는 실제 GOP 유사환경 적응이 목적이므로 phase1보다 작은 객체를 더 허용한다.

기본 출력 구조:
    data/gop_raw_filtered/single/{class}/{rgb|tir}/img
    data/gop_raw_filtered/single/{class}/{rgb|tir}/label
    data/gop_raw_filtered/empty/{rgb|tir}/img
"""
...
DEFAULT_PLANS = [
    SelectionPlan("person_rgb_raw", "single", 0, "rgb", 2500, "p3_c0_rgb_raw", exclude_prefixes=("fp_",)),
    SelectionPlan("person_tir", "single", 0, "tir", 2500, "p3_c0_tir"),
    SelectionPlan("boar_rgb", "single", 1, "rgb", 1500, "p3_c1_rgb"),
    SelectionPlan("boar_tir", "single", 1, "tir", 1500, "p3_c1_tir"),
    SelectionPlan("deer_rgb", "single", 2, "rgb", 1500, "p3_c2_rgb"),
    SelectionPlan("deer_tir", "single", 2, "tir", 1500, "p3_c2_tir"),
    SelectionPlan("non_target_rgb", "single", 3, "rgb", 1000, "p3_c3_rgb"),
    SelectionPlan("empty_rgb", "empty", None, "rgb", 1000, "p3_empty_rgb"),
    SelectionPlan("empty_tir", "empty", None, "tir", 1000, "p3_empty_tir"),
]
...
def _source_dirs(source_root: Path, plan: SelectionPlan) -> tuple[Path, Path | None]:
    if plan.group == "empty":
        return source_root / "empty" / plan.modality / "img", None
    base = source_root / "single" / str(plan.class_id) / plan.modality
    return base / "img", base / "label"


def _collect_candidates(plan: SelectionPlan, args: argparse.Namespace) -> tuple[list[Sample], dict[str, int]]:
    source_root = Path(args.source_root)
    img_dir, label_dir = _source_dirs(source_root, plan)
    stats = {
        "images": 0,
        "missing_label": 0,
        "prefix_filtered": 0,
...
def _quality_key(sample: Sample) -> tuple[float, float, float]:
    # phase3는 원거리/소형 객체 대응이 중요하므로 phase1보다 작은 bbox를 우선한다.
    target_area = 0.025 if sample.max_area_ratio > 0 else 0.0
...
def _select_samples(samples: list[Sample], count: int, rng: random.Random) -> list[Sample]:
    ...
    selected = rng.sample(ranked[:top_pool_size], count)
    return sorted(selected, key=lambda sample: sample.image.name)


def _target_dirs(target_root: Path, plan: SelectionPlan) -> tuple[Path, Path | None]:
    if plan.group == "empty":
        return target_root / "empty" / plan.modality / "img", None
    base = target_root / "single" / str(plan.class_id) / plan.modality
    return base / "img", base / "label"
...
def _copy_samples(
    ...
    dst_img_dir, dst_label_dir = _target_dirs(target_root, plan)
    ...
    for index, sample in enumerate(samples):
        ...
        if not overwrite and dst_image.exists():
            raise FileExistsError(f"출력 이미지가 이미 있습니다: {dst_image}")
        shutil.copy2(sample.image, dst_image)

        if sample.label is not None and dst_label_dir is not None:
            dst_label = dst_label_dir / f"{stem}.txt"
            if not overwrite and dst_label.exists():
...
def build_phase3_subset(args: argparse.Namespace) -> None:
    rng = random.Random(args.seed)
    target_root = Path(args.target_root)
    print(f"phase3 subset 생성: {args.source_root} -> {target_root}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="gop_raw 품질 필터링 phase3 subset 생성")
    parser.add_argument("--source-root", default="data/gop_raw", help="원본 gop_raw 루트")
    parser.add_argument("--target-root", default="data/gop_raw_filtered", help="출력 gop_raw 루트")
    ...
    parser.add_argument("--min-area-ratio", type=float, default=0.0004, help="최소 bbox 면적 비율")
    parser.add_argument("--max-area-ratio", type=float, default=0.70, help="최대 bbox 면적 비율")
    parser.add_argument("--min-width-ratio", type=float, default=0.005, help="최소 bbox 너비 비율")
    parser.add_argument("--min-height-ratio", type=float, default=0.01, help="최소 bbox 높이 비율")
    ...
    parser.add_argument("--min-blur", type=float, default=0.0, help="Laplacian blur score 하한")
    parser.add_argument("--min-brightness", type=float, default=2.0, help="평균 밝기 하한")
    parser.add_argument("--max-brightness", type=float, default=253.0, help="평균 밝기 상한")
    return parser.parse_args()


def main() -> None:
    build_phase3_subset(parse_args())
