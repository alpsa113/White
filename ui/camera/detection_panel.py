...
from ui.styles import DETECTION_PANEL_CSS
...
<div style="display:flex;align-items:center;gap:0.7rem;padding:0.55rem 0.75rem;
    ...
    <div style="flex:1;min-width:0;">
        <div style="display:flex;align-items:baseline;justify-content:space-between;gap:0.5rem;">
            ...
            <div style="font-size:0.8rem;color:#9aa4b2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">
                {cam_name}
            </div>
        </div>
...
def render_detection_panel() -> None:
    ...
    st.markdown(DETECTION_PANEL_CSS, unsafe_allow_html=True)
    ...
    with st.container(key="detection_panel_wrap"):
        st.markdown("**탐지 이력**")
        with st.container(key="detection_panel_list"):
            if not logs:
