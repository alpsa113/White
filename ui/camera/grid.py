...
def render_camera_grid(cameras: list[dict], eo_video_slots: dict, tir_video_slots: dict, cols_per_row: int) -> None:
    ...
    for row_start in range(0, len(cameras), cols_per_row):
        ...
        for col, cam in zip(cols, row_cams):
            with col:
                render_camera_card(cam, eo_video_slots, tir_video_slots)
