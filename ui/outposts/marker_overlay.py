"""
ui/outposts/marker_overlay.py — 지도 위 초소 마커 오버레이 (관제 지도 탭) +
마커 선택 상태 공용 로직 (설정 페이지 지도 미리보기와도 공유)

색상 규칙 (설정 페이지 지도 미리보기 · 관제 지도 탭 공통):
...
갱신됩니다. 이 상태는 세 곳 모두에서 같은 뜻으로 쓰입니다: 관제 지도 탭의
왼쪽 CCTV 요약, 두 화면의 마커 색상, 그리고 '카메라 화면' 탭에 표시할
카메라 필터(views/dashboard.py가 이 값을 직접 읽어 선택된 카메라들만
그리드로 보여줍니다 — 스포트라이트로 확대하는 것이 아니라 기존 그리드
배치 그대로 대상만 좁히는 것입니다).

`render_marker()`(클릭 가능한 마커 버튼)는 대시보드 "관제 지도" 탭에서만
사용합니다. 설정 페이지의 지도 미리보기는 새 마커를 클릭으로 추가해야 해서
`streamlit_image_coordinates`가 이미 클릭을 가로채므로, 그 위에 마커
버튼을 또 겹치지 않고 대신 PIL로 마커를 이미지에 그려 넣어 보여주기만
하고(`ui/outposts/editor.py` 참고), 선택/해제·삭제는 목록의 별도 버튼으로
`toggle_selection()`/`services.outposts.remove_marker()`를 직접 호출합니다.

점멸(사람 탐지)은 색상과는 별개 축입니다 — 선택 여부와 무관하게 현재
색상(빨강/하늘색) 그대로 깜빡이는 효과만 덧붙습니다. 정지(⏹) 아이콘은
점멸 중일 때만 그리는 화면(관제 지도 탭)에서 선택적으로 사용합니다.
"""
