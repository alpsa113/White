...
이 디렉터리는 YOLO26-M COCO pretrained checkpoint를 로드하기 위한 provider 코드 위치입니다.

현재 provider는 공식 Ultralytics 패키지 소스를 vendor 형태로 포함합니다.
...
- C3/C4까지만 실행 가능한 layer graph여야 하며, 학습 forward에서 Neck/Head는 호출하지 않습니다.
...
pretrained weight 파일은 이 디렉터리에 저장하지 않습니다.
