...
open_browser() {
  local url="http://127.0.0.1:5173/login"
  if command -v open >/dev/null 2>&1; then
    open "$url"                  # macOS
  elif command -v xdg-open >/dev/null 2>&1; then
    xdg-open "$url"              # Linux
  elif command -v cmd.exe >/dev/null 2>&1; then
    # Git Bash(MINGW64)/Cygwin에서는 MSYS 경로 자동 변환 때문에
    # start "" "url" 인자가 깨질 수 있어 변환을 꺼준다.
    MSYS2_ARG_CONV_EXCL="*" cmd.exe /c start "" "$url"
  elif command -v start >/dev/null 2>&1; then
    start "$url"                 # Windows (native cmd)
  else
    echo "[INFO] 브라우저에서 $url 접속"
  fi
}
