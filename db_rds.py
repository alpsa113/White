...
@lru_cache(maxsize=1)
def get_engine():
    """SQLAlchemy 엔진을 생성하고 프로세스 전체에서 재사용합니다(환경변수 기반 설정)."""
    dialect = os.environ.get("RDS_DIALECT", "mysql+pymysql")
    ...
    # pymysql 기본값은 연결 타임아웃이 무제한에 가까워, 네트워크가 잠깐 불안정하면 탐지
    # 스레드 하나가 RDS 응답을 하염없이 기다리며 묶일 수 있습니다. 짧게 강제합니다.
    # pool_timeout: 커넥션 풀이 다 찼을 때 새 연결을 기다리는 시간도 함께 제한합니다.
...
def get_last_init_error() -> str | None:
    """가장 최근 init_db() 실패 시의 에러 메시지를 반환합니다(성공/미실행 시 None)."""
